...
classifier with more predictive power for high frequency malignancies.
The integration eventually results in the creation of the Minority &
...
For this tutorial, the `tidyverse` and  `MnM` package need to be installed and loaded.
The installation file for MnM is within this folder and is called:
MnM_1.0.1.tar.gz.

1)  Download package `remotes` and use function *install_local* to
    install the MnM package from file 'MnM_1.0.1.tar.gz'. Make sure to
...
if(!require("MnM")) {
  remotes::install_local("./MnM_1.0.1.tar.gz", dependencies = T)
}
library("tidyverse")
library("MnM")
```
