...
# d
colnames(countDataRef)

```

For M&M to function, the sample IDs should be present both within the
count dataset and within the metadata. Where the count data contains the
sample IDs within the column names, the metadata should contain the
...
```{r}
# a 
"Information is within column sampleID"

# provided code
all(metaDataRef$sampleID %in% colnames(countDataRef))

# b
all(colnames(countDataRef) %in% metaDataRef$sampleID)
...
Check the first entries within the metadata. Then try to find out which
...
#a 
"Tumor subtype labels are in 'tumorSubtype'"

#b 
"Tumor type labels are in 'tumorType'"

#c
"Domain labels in 'Domain'"
```
...
```{r}
# example code
table(metaDataRef$tumorType)[table(metaDataRef$tumorType) < 3]

table(metaDataRef$tumorSubtype)[table(metaDataRef$tumorSubtype) < 3]
...
a)  How is the number of total RNA counts distributed per sample? Run
    ...
    barplot that the data is normalized?
...
"Every sample has the same total number or RNA counts, caused by the normalization of expression values to RNA-transcripts per million reads. This accounts for differences in the total number of RNA-transcripts in different samples."
...
"There are no clear outliers, there appears to be a normal distribution present. Important to note is that a log-scale should be used for this visualization, otherwise the distribution will look a lot different. "
...
g\. Run the *createUMAPcohort* function. Store the results in the object
'dataUMAPList'. Please note the function will specify that there are no
abbreviations supplied - this is correct. In case we would like to use
different, abbreviated labels within the UMAP, they can be supplied
within the parameter 'abbreviations'.
...
# b
head(minority$classifications)
"predict needs to be equal to originalCall"

# c
correctClassifications <- minority$classifications %>% filter(originalCall == predict) %>% nrow()
# accuracy
correctClassifications / nrow(minority$classifications) 
```
...
# c
names(majority) # The reducedFeaturesList is missing, for the rest the same as for the minority classifier.
...
# b
"M&M does not perform better in this case, Minority classifier is better "
"However, for other tumor entities the Majority classifier is better, so that integration is still preferred"
```
...
# b 
"subtype determines which level will be used. We need to put subtype = T to make sure only look at the classification within the B-ALL subtypes."

# provided code
# subtypeBALLClassifications <- integrateMM(minority,
#                         majority,
#                         subtype)
...
correctClassifications <- predictionsMMFinal %>% filter(originalCall == predict) %>% nrow() 
correctClassifications / nrow(predictionsMMFinal) 

# e

correctClassifications <- predictionsMMFinal %>% filter(probability1 > 0.72,
                              originalCall == predict) %>% nrow() 
allRecalledClassifications <- predictionsMMFinal %>% filter(probability1 > 0.72)  %>% nrow()
correctClassifications / allRecalledClassifications 
...
but variable names need to be altered so as to match the names used
...
# 
predictionsMMTest %>% filter(originalCall == predict)  %>% dim()# 90
90 / nrow(predictionsMMTest) # 86

"Very comparable accuracy, so no apparent overfitting"
```
...
[Zenodo](https://zenodo.org/records/11575098).

## Q16:

a)  Download the 'createdModelsMinority.rds' and
    'createdModelsMajority.rds' from the Zenodo directory and move them
    to the tutorial input directory (DataTutorial/input/).
b)  Read in the rds-objects using the code from below. Please make sure
    you have put the models correctly within the input directory of the
    tutorial directory.
c)  Run the code in *newPredictionsMinority* with the filled out
    required parameters to generate minority classifications for the
    test samples stored in 'countDataTest'. Save the models within the
    directory 'testResultsPretrainedModel', within the output folder of
    today's date. Use the provided code, but alter the *outputDir*
    parameter to reflect today's date.
d)  Run the code in *newPredictionsMajority* with the filled out
    required parameters to generate majority classifications for the
    test samples stored in 'countDataTest'. Be sure to save the models
    within the directory 'testResultsPretrainedModel', within the output
    folder of today's date. Use the provided code, but alter the
    *outputDir* parameter to reflect today's date.
e)  Integrate the two models' classifications using the function
    *integrateMM* again by filling out the required parameters, in the
    same way as for Q14d.
f)  Add the originalCall column again, and check how many errors were
    made by M&M. Which classifier performs better, the pretrained one or
    the classifier trained only on the B-ALL samples? Does that make
    sense?

```{r}
# b
modelsMinority <- readRDS("~/Downloads/createdModelsMinority.rds")
modelsMajority <- readRDS("~/Downloads/createdModelsMajority.rds")
...
# e
predictionsMMTestList <- integrateMM(predictionsTestMinority,
...
correctClassifications <- predictionsMMTest %>% filter(originalCall == predict)  %>% nrow() 
correctClassifications / nrow(predictionsMMTest) 

# The new classifier for only B-ALL subtype classifications performs better. This comes as no surprise, as the full model also has to discriminate 51 other tumor types and many tumor subtypes. However, the difference in the error count is limited. Therefore, the performance of M&M is considered remarkable. 
