...
# accuracy
240 / nrow(minority$classifications) # 90%
...
predictionsMMFinal %>% filter(originalCall == predict) %>% dim() # 228
228 / nrow(predictionsMMFinal) # 85%

# e

predictionsMMFinal %>% filter(probability1 > 0.72,
                              originalCall == predict) %>% dim() # 170 
predictionsMMFinal %>% filter(probability1 > 0.72) # 174
170 / 174 # 98%
...
predictionsMMTest %>% filter(originalCall == predict)  %>% dim()# 89
89 / nrow(predictionsMMTest) # 86

# The new classifier for only B-ALL subtype classifications performs better. This comes as no surprise, as the full model also has to discriminate 51 other tumor types and many tumor subtypes. However, the difference in the error count is negligible (1 sample). Therefore, the performance of M&M is considered remarkable. 
